...
$script = Join-Path $invocationPath "RunTestCasesByBinariesAndFilterLinux.ps1"

$binaries = @(
    "MS-SMBD_ServerTestSuite.dll"
)

& $script -Binaries $binaries -Filter $Filter -DryRun:$DryRun.IsPresent -BinPath $BinPath
exit $LASTEXITCODE
